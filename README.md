...
**Participation gate (since 9 September 2026).** On top of the confidence threshold, a candidate must show **opening-range volume ≥ 1.5× the average of the same 9:30–9:34 window over the previous 20 trading days** (`MIN_VOL_RATIO_ENTRY`). Volume used to be only a bonus; now it is a requirement. Three months of live data showed that the technical signals alone don't separate winners from losers, while participation does — "no news + thin volume" setups lost −$7.2k at 39% win and produced 8 of 11 hard stops — and the rule was validated out-of-sample on the 631-trade backtest (profit factor 1.02 → 1.11, P&L ×4, drawdown −25%, average win +22%). A candidate that fails it is logged as `no_participation` and shown as **NO VOL** in the dashboard. If the 20-day history is unavailable the candidate is rejected (same semantics as the backtest). A news catalyst can be allowed to bypass the gate (`CATALYST_BYPASSES_VOL_GATE`) — off for now, pending live validation.
...
1. **Gate di partecipazione all'entrata — DEPLOYATO il 9/09** con `vol_ratio ≥ 1.5` (solo volume). Il ramo volume è validato su due campioni indipendenti: in-sample sui 69 trade live e **out-of-sample sul backtest a 631 trade** (PF 1.02 → 1.11, P&L ×4, DD −25%, avg win +22% — vedi changelog 8/09). È la prima regola d'ingresso a reggere fuori campione. Il ramo **catalyst** (`CATALYST_BYPASSES_VOL_GATE`, spento) **non** è backtestabile (il backtest non ha news): da valutare sui prossimi trade live — se i nomi con news ma volume basso che il gate ora scarta si rivelano vincitori, si accende il flag. **Da monitorare:** numero di trade/mese (atteso ~−27%), frequenza degli scarti `NO VOL` in dashboard, e se qualche vincitore evidente viene scartato per volume.
...
- `[fix]` **Calcolo volumi verificato per il gate**: finestra di oggi (`bars < 9:35`) = finestra storica (`bars < 9:35`, 20 gg) = backtest (5 barre 9:30–9:34, media 20 gg), stesso feed IEX → la soglia si trasferisce. Aggiunta la normalizzazione della finestra **troncata** (4% dei casi live anche dopo il retry): una barra 9:34 non ancora pubblicata deflazionava il ratio del ~20% e avrebbe prodotto falsi scarti; ora il ratio è normalizzato alla finestra attesa, il `vol_today` nel log resta grezzo.
- `[fix]` **Gap pre-market da una sola sorgente**: `prev_close` veniva dalla daily bar Alpaca (IEX = ultimo print, non l'asta di chiusura) e il prezzo pre-market da Yahoo — due sorgenti nella stessa frazione. Ora, quando il pre-market viene da Yahoo, il riferimento è la chiusura ufficiale Yahoo (stessa chiamata), e lo stesso `prev_close` alimenta gate pre-open, check "gap invertito" e gap retention. Fallback IEX: coerente IEX-vs-IEX.
- `[feat]` Dashboard: asse X del P&L giornaliero diradato e grafico a larghezza piena (le 66 date si sovrapponevano); colonna **Volume** nel trade log mostra il multiplo reale (×2.3) con tooltip sulla definizione; badge/filtro **NO VOL** per gli scarti da gate.
- `[feat]` Backtest: `min_vol_ratio` (gate di partecipazione) aggiunto allo sweep `--entry-cap`.
- `[exp]` *Nota:* la baseline di questo run (universo a 68, +$4.7k) è più bassa di quella del 5/08 (70 nomi, +$12.3k). L'unica differenza è INTC/RKLB rimossi — confuso dall'effetto sostituzione (il backtest riempie gli slot in ordine di universo), quindi non è una prova che fossero buoni, ma il taglio poggiava su 5 trade live. Un backtest per-ticker lo chiarirebbe.
